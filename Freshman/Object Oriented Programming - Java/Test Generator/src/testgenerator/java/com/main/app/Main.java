...
public class Main implements MainMethods {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, CloneNotSupportedException, InputMismatchException {
        ...
        boolean bol = true;
        ArrayList< Question > allQuestions = MainMethods.readEverythingFromBinaryFile();

        /*ArrayList< Question > allQuestions = new ArrayList<>();
        Question q1 = new AmericanQuestion("What is karen's last name?", "Kalif");
        Question q2 = new OpenFieldQuestion("How many days in a year?", "365");
        Question q3 = new OpenFieldQuestion("How many months in a year?", "12");
        Question q4 = new AmericanQuestion("How many months in a year?", "12");
        Question q5 = new AmericanQuestion("In which year was Facebook first launched?", "2004");
        allQuestions.add(q1);
        allQuestions.add(q2);
        allQuestions.add(q3);
        allQuestions.add(q4);
        ...
        do {
            try {
                ...
                switch (choice) {
                    ...
                    case 2 -> {
                        s.nextLine();
                        String answer, type = null, text = null;
                        boolean boll = true;
                        do {
                            try {
                        ...
                        if (type.equalsIgnoreCase("american")) {
                            ...
                            s.nextLine();
                            ArrayList< String > newList = new ArrayList<>(numOfWrongAnswers + 1);
                            newList.add(0, correctAnswer);
                            for (int i = 1; i < numOfWrongAnswers + 1; i++) {
                                newList.add(i, s.nextLine());
                            }
                    ...
                    case 4 -> {
                        ...
                        if (allQuestions.get(id) instanceof OpenFieldQuestion) {
                            ((OpenFieldQuestion) allQuestions.get(id)).setAnswer(newAnswer);
                            ...
                            if (specificAnswer == 'a') {
                                that.setRightAnswer(newAnswer);
                                ((AmericanQuestion) allQuestions.get(id)).setSpecificAnswer(newAnswer, 0);
                            } else {
                                ((AmericanQuestion) allQuestions.get(id)).setSpecificAnswer(newAnswer, (int) specificAnswer - 97);
                            }
                            System.out.println("Answer updated successfully");
                        }
                    }
                                    ...
                                    if (allQuestions.get(question) instanceof OpenFieldQuestion) {
                                        ...
                                        do {
                                            ...
                                            if (ch.equalsIgnoreCase("N")) {
                                                MainMethods.addAmericanQuestionLastAnswers(that);
                    ...
                    case 7 -> {
                        ...
                        for (int i = 0; i < amountOfQuestions; i++) {
                            Controller.automaticExam(i, allQuestions, newTest);
                        }
